...
import {
  Phone, Mail, CheckCircle2, Hammer, Shield, Layers, ArrowRight, Star, Flame
} from "lucide-react";
...
function PhoneHighlight({children}){ return <span className="text-orange-500 font-semibold">{children}</span>; }
...
function DragonLogo({ className="", compact=false }){
  return (
    <div className={cx("flex items-center", compact ? "gap-2" : "gap-3", className)}>
      <div className={cx("font-black tracking-tight", compact ? "text-xl" : "text-2xl")}>
        DRAGON <span className="text-orange-500">PAVING</span>
      </div>
    </div>
  );
}
...
function Pill({children, tone="neutral"}){
  ...
  return <span className={cx("inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium shadow-sm backdrop-blur", styles[tone] || styles.neutral)}>{children}</span>;
}
...
function TopNavLink({to, children}){
  return (
    <RRNavLink to={to} className={({isActive}) => cx(
    ...
    <motion.div whileHover={{y:-4}} transition={{type:"spring",stiffness:250,damping:20}}
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          {to ? <div className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-slate-950">View details <ArrowRight className="h-4 w-4" /></div> : null}
        </div>
      </div>
    </motion.div>
  );

  return to ? <Link to={to} className="block rounded-2xl focus:outline-none focus:ring-2 focus:ring-orange-200">{Card}</Link> : Card;
}
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white/80 p-6 shadow-sm backdrop-blur">
      <div className="flex items-center gap-1 text-orange-600">
        {Array.from({length:stars}).map((_,i)=>(<Star key={i} className="h-4 w-4" />))}
      </div>
...
function ContactField({label, value, icon:Icon, href}){
  ...
  return href ? <a href={href} className="block rounded-2xl focus:outline-none focus:ring-2 focus:ring-orange-200">{content}</a> : content;
}
    ...
    <div className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/70 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <button className="inline-flex items-center rounded-xl border border-white/15 bg-white/10 p-2 text-white shadow-sm md:hidden" onClick={()=>setMenuOpen(v=>!v)} aria-label="Toggle menu">
    ...
    <main>
      <section className="mx-auto max-w-6xl px-4 pt-10 sm:px-6 sm:pt-14">
        <div className="grid items-center gap-10 lg:grid-cols-2">
          <div>
            <div className="flex flex-wrap gap-2">
              <Pill>Licensed & insured*</Pill><Pill>Free estimates</Pill><Pill>Commercial</Pill>
            </div>
            ...
            <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/10 shadow-sm backdrop-blur">
              <div className="p-6 sm:p-8">
                ...
                <div className="mt-6 grid gap-3">
                  {["Asphalt paving","Sealcoating","Asphalt milling","Concrete services"].map((t)=>(
...
function ServicesOverviewPage({services}){
  return (
    <main>
      <section className="mx-auto max-w-6xl px-4 pt-10 sm:px-6 sm:pt-14">
        <SectionTitle eyebrow="Services" title="Commercial asphalt & concrete" sub="Click a service for details, typical scopes, and what to expect on-site." />
        <div className="mt-10 grid gap-4 lg:grid-cols-2">
          {services.map((s)=>(
            <ServiceCard key={s.slug} icon={s.cardIcon} title={s.title} points={s.points} to={`/services/${s.slug}`} />
          ))}
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-white/80 p-6 shadow-sm backdrop-blur">
              ...
              <ul className="mt-3 space-y-2">
                {service.scope.map((p)=>(
                  <li key={p} className="flex items-start gap-2 text-sm text-slate-700">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 text-orange-600" /><span>{p}</span>
                  </li>
                ))}
              </ul>
            </div>
              ...
              <form onSubmit={onSubmit} className="grid gap-3">
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="grid gap-1">
                    ...
                    <input value={form.name} onChange={(e)=>setForm(p=>({...p,name:e.target.value}))}
                      className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-950 shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-200"
                ...
                <label className="grid gap-1">
                  ...
                  <textarea value={form.message} onChange={(e)=>setForm(p=>({...p,message:e.target.value}))}
                    className="min-h-[140px] rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-950 shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-200"
                    placeholder="Example: 4,000 sq ft parking lot needs milling + overlay. Looking to schedule in March."
                    required />
                </label>
